"""
Train GINE+MLP (no local/global split) with auxiliary heads from trunk state.
...
Aux heads read the GINE encoder trunk representation (state_flat) directly,
while voltage prediction still uses RealMLP(state_flat)->v_pred_n.
Each task uses a per-head MLP classifier (VREG4B-style widths).
"""
...
class GINEPlusMLPAux(nn.Module):
    ...
    def forward_train(self, batch: Data) -> tuple[torch.Tensor, list[torch.Tensor], list[torch.Tensor]]:
        ...
        reg_logits = [head(state_flat) for head in self.aux_reg_heads]
        cap_logits = [head(state_flat) for head in self.aux_cap_heads]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train GINE+MLP with aux heads on trunk encoder state.")
    ...
    p.add_argument("--out_dir", type=str, default="gine_plus_mlp_aux_trunk_complex_8500")
...
def main() -> None:
    ...
    report = {
        "task": "GINE+MLP with aux heads on trunk state (no local/global voltage decomposition)",
