"""
Refine exploration: 8 configs from Block 4 insights.
Target: beat MAE 0.006114 (light_emb_h96_phase_onehot_depth3 + Load-type).
Uses 30% data, corrected MAE/RMSE. No dropout.
Explores: phase_onehot+norm+depth, wider phase_onehot, phase_subgraph (fixed), phase_onehot+huber.
"""
import os
import re
import numpy as np
...
# 8 configs: (name, n_emb, e_emb, h_dim, n_layers, use_norm, use_phase_subgraphs, use_phase_onehot, use_huber)
CANDIDATES = [
    ("light_emb_h96_phase_onehot_norm_depth3", 16, 8, 96, 3, True, False, True, False),
    ("light_xwide_emb_phase_onehot_norm_depth3", 16, 8, 128, 3, True, False, True, False),
    ("light_xwide_emb_phase_onehot_depth3_h160", 16, 8, 160, 3, False, False, True, False),
    ("light_emb_h96_phase_onehot_depth3_h112", 16, 8, 112, 3, False, False, True, False),
    ("light_emb_h96_phase_subgraph", 16, 8, 96, 2, False, True, False, False),
    ("light_emb_h96_phase_subgraph_depth3", 16, 8, 96, 3, False, True, False, False),
    ("light_emb_h96_phase_onehot_huber", 16, 8, 96, 2, False, False, True, True),
    ("light_xwide_emb_phase_onehot_huber", 16, 8, 128, 3, False, False, True, True),
]
...
def _batch_phase_subgraph(data, phase_edges):
    ...
    if isinstance(data, Batch):
        ...
        for p in range(3):
            ...
            if ei.numel() == 0:
                out[f"edge_index_{p}"] = ei.to(dev)
                out[f"edge_attr_{p}"] = ea.to(dev)
                out[f"edge_id_{p}"] = eid.to(dev)
            else:
                ei_list = [ei.to(dev) + g * N for g in range(n_graphs)]
                out[f"edge_index_{p}"] = torch.cat(ei_list, dim=1)
                out[f"edge_attr_{p}"] = ea.repeat(n_graphs, 1).to(dev)
                out[f"edge_id_{p}"] = eid.repeat(n_graphs).to(dev)
        return out
    return data
...
def _log_skip_reason(out_dir, edge_csv, node_csv):
    if not hasattr(_log_skip_reason, "_logged"):
        missing = [p for p in [edge_csv, node_csv] if not os.path.exists(p)]
        print(f"  [SKIP] Missing: {missing}. Run GNN2 to generate datasets.")
        _log_skip_reason._logged = True


def train_one(out_dir, feature_cols, ds_label, cfg_name, n_emb, e_emb, h_dim, n_layers,
              use_norm, use_phase_subgraphs, use_phase_onehot, use_huber, data_frac=0.30):
    seed_all(SEED)
...
def main():
    print("=" * 70)
    print(f"REFINE EXPLORATION: {int(DATA_FRAC*100)}% data, 8 configs × 3 datasets")
    print("Target: beat MAE 0.006114 | Phase onehot+norm+depth | Phase subgraph | Huber")
    print(f"Working dir: {os.path.abspath('.')}")
    for d, _, _ in DATASETS:
        ec, nc = os.path.join(d, "gnn_edges_phase_static.csv"), os.path.join(d, "gnn_node_features_and_targets.csv")
        print(f"  {d}: {'OK' if os.path.isdir(d) else 'MISSING'} (edge={os.path.exists(ec)}, node={os.path.exists(nc)})")
    print("=" * 70)
    results = []
    for cfg in CANDIDATES:
        cfg_name, n_emb, e_emb, h_dim, n_layers, use_norm, use_phase_subgraphs, use_phase_onehot, use_huber = cfg
        for out_dir, feature_cols, ds_label in DATASETS:
            print(f"\n>>> {cfg_name} + {ds_label} (n={n_layers} h={h_dim} norm={use_norm} phase_sg={use_phase_subgraphs} phase_oh={use_phase_onehot} huber={use_huber})")
            try:
                mae, rmse, best_ep = train_one(out_dir, feature_cols, ds_label, cfg_name, n_emb, e_emb, h_dim, n_layers,
                                              use_norm, use_phase_subgraphs, use_phase_onehot, use_huber, data_frac=DATA_FRAC)
                if mae is not None:
