...
Scenario structure: 1000 scenarios with different (P_load, Q_load, P_pv); for each scenario,
...
def generate_gnn_snapshot_dataset_deltav(
    ...
    """
    Generate delta-V dataset with 24h profile alignment:
    - Each scenario: different (P_load, Q_load, P_pv) from BASELINE × RANGES
    - For each scenario: k timesteps sampled from 24h profile via select_times_three_profiles
    - At each t: vmag_zero = solve with mL[t], PV=0; vmag_with_pv = solve with mL[t], mPV[t]
    - Target = vmag_with_pv - vmag_zero (matches test-time temporal structure)
    ...
    for s in range(n_scenarios):
        ...
        prof_load = mL
        prof_pv = mPV
        prof_net = (P_load * mL) - (P_pv_baseline * mPV)
        ...
        print(f"[scenario {s+1}/{n_scenarios}] kept={kept} skip_nonconv={skipped_nonconv} skip_badV={skipped_badV} Pload={P_load:.1f} Qload={Q_load:.1f} Ppv_base={P_pv_baseline:.1f} times={len(times)}")
    ...
    print(f"  Features per node: electrical_distance_ohm, m1_p, m1_q, m2_p, m2_q, m4_p, m4_q, m5_p, m5_q, q_cap, p_pv, q_pv_kvar, p_sys_balance, q_sys_balance, vmag_zero_pv_pu")
