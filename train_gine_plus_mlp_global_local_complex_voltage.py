"""
Train a GINE encoder with local voltage path + global MLP correction path.

Goal:
  - Keep the same task/output protocol as train_gine_plus_mlp_complex_voltage.py
    and train_compare_mlp_cvnn_complex_voltage.py:
      predict flattened [V_re, V_im] and report |V|/angle metrics.
  - Add a local per-node voltage path while preserving the same global RealMLP
    head used in the plain GINE+MLP experiment.

Pipeline:
  X_node(P,Q) --GINE--> h_i, state_i(2 dims/node)
    local path:  h_i -> [V_re, V_im]_local
    global path: flatten(state_i) -> RealMLP -> Delta[V_re, V_im]_global
  final output = local + global_delta
"""
...
class RealMLP(nn.Module):
    # Exactly the same architecture as the MLP-only baseline.
...
class GINEEncoder(nn.Module):
    ...
    def forward(self, batch: Data) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        h_batched = h.view(batch.num_graphs, self.n_nodes, self.hidden_dim)
        state_batched = state.view(batch.num_graphs, self.n_nodes, self.state_dim)
        return h_batched, state_batched


class GINEPlusMLPGlobalLocal(nn.Module):
    def __init__(self, encoder: GINEEncoder, mlp_hidden: int, n_nodes: int):
        super().__init__()
        self.encoder = encoder
        self.local_head = nn.Linear(encoder.hidden_dim, 2)
        self.global_mlp = RealMLP(in_dim=2 * n_nodes, out_dim=2 * n_nodes, hidden=mlp_hidden)
        self.n_nodes = int(n_nodes)

    def forward(self, batch: Data) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        h, state = self.encoder(batch)
        local = self.local_head(h)  # [B, N, 2]
        delta = self.global_mlp(state.reshape(state.size(0), 2 * self.n_nodes)).view(state.size(0), self.n_nodes, 2)
        pred = local + delta
        return pred.reshape(pred.size(0), 2 * self.n_nodes), local.reshape(local.size(0), 2 * self.n_nodes), delta.reshape(delta.size(0), 2 * self.n_nodes)

    def forward_voltage(self, batch: Data) -> torch.Tensor:
        pred, _local, _delta = self.forward(batch)
        return pred
...
@torch.no_grad()
def _evaluate(model: GINEPlusMLPGlobalLocal, dl: DataLoader, device: torch.device, y_mean: torch.Tensor, y_std: torch.Tensor) -> dict[str, float]:
    model.eval()
    preds = []
    tgts = []
    for batch in dl:
        batch = batch.to(device)
        yp_n = model.forward_voltage(batch)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train GINE with local path + global MLP correction for complex voltage.")
    ...
    p.add_argument("--out_dir", type=str, default="gine_plus_mlp_global_local_complex_8500")
...
def main() -> None:
    ...
    if int(args.state_dim) != 2:
        raise ValueError("This experiment enforces state_dim=2 to match the plain GINE+MLP setup.")
    ...
    model = GINEPlusMLPGlobalLocal(encoder=encoder, mlp_hidden=int(args.hidden_mlp), n_nodes=n_nodes).to(device)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for batch in dl_tr:
            ...
            pred_n, _local_n, _delta_n = model(batch)
        ...
        if val_loss < best_val:
            ...
            bad += 1
        if ep == 1 or ep % 10 == 0:
            print(f"[gine+mlp+local-global] epoch {ep:4d}/{args.epochs} val_mse_norm={val_loss:.6f} best={best_val:.6f}", flush=True)
        if bad >= args.patience:
            print(f"[gine+mlp+local-global] early stopping at epoch {ep}", flush=True)
            break
    ...
    ckpt_path = out_dir / "gine_plus_mlp_global_local_best.pt"
    torch.save(
        {
    ...
    report = {
        "task": "GINE(PQ)->local path + global RealMLP correction -> complex voltage [V_re,V_im]",
        ...
        "model": {
            "type": "gine_plus_mlp_global_local",
    ...
    report_path = out_dir / "gine_plus_mlp_global_local_report.json"
    report_path.write_text(json.dumps(report, indent=2), encoding="utf-8")

    print("\n=== Training complete ===", flush=True)
    print(f"Report: {report_path}", flush=True)
    print(
        f"GINE+MLP(global+local): |V| MAE={result.test_mae_vmag:.6f} pu, "
