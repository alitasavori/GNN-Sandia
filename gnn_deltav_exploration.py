...
# Best configs from Load-type (dataset 3) — PFIdentityGNN only, no phase_subgraph
# (name, n_emb, e_emb, h_dim, n_layers, use_norm, use_phase_onehot)
CANDIDATES = [
...
def train_one(out_dir, feature_cols, target_col, cfg_name, n_emb, e_emb, h_dim, n_layers,
    ...
    if not os.path.exists(edge_csv) or not os.path.exists(node_csv):
        print(f"  [SKIP] Missing {out_dir}. Run GNN2 fourth dataset block first.")
...
def main():
    print("=" * 70)
    print("DELTA-V EXPLORATION: Best Load-type configs on fourth dataset (vmag_delta_pu)")
    ...
    for cfg in CANDIDATES:
        ...
        print(f"\n>>> {cfg_name} + Delta-V (n={n_layers} h={h_dim} norm={use_norm} phase_oh={use_phase_onehot})")
        try:
    ...
    if len(df_res) > 0:
        ...
        best = df_res.iloc[0]
        print(f"\nBest: {best['Config']} + Delta-V | MAE={best['MAE']:.6f} | RMSE={best['RMSE']:.6f}")
    else:
