"""
Boost exploration: 10 configs derived from Block 3 (narrow) best results.
Target: beat MAE 0.006161 (light_xwide_emb_depth3 + Load-type).
Uses 30% data, corrected MAE/RMSE. No dropout.
Explores: deeper, wider, phase_onehot+depth, norm+depth, Huber loss, LR scheduler.
"""
import os
import re
import numpy as np
...
# 10 configs: (name, n_emb, e_emb, h_dim, n_layers, use_norm, use_phase_onehot, use_huber)
CANDIDATES = [
    ("light_xwide_emb_depth4", 16, 8, 128, 4, False, False, False),
    ("light_xwide_emb_depth3_h160", 16, 8, 160, 3, False, False, False),
    ("light_xwide_emb_phase_onehot_depth3", 16, 8, 128, 3, False, True, False),
    ("light_emb_h96_phase_onehot_depth3", 16, 8, 96, 3, False, True, False),
    ("light_xwide_emb_norm_depth3", 16, 8, 128, 3, True, False, False),
    ("light_emb_h96_norm_depth3", 16, 8, 96, 3, True, False, False),
    ("wide_shallow_h160_depth4", 8, 4, 160, 4, False, False, False),
    ("light_xwide_emb_depth3_h192", 16, 8, 192, 3, False, False, False),
    ("light_emb_h96_phase_onehot_norm", 16, 8, 96, 2, True, True, False),
    ("light_xwide_emb_huber", 16, 8, 128, 3, False, False, True),
]
    ...
    if use_phase_onehot:
        ...
        if phase_map is None:
            if not hasattr(train_one, "_warned_phase"):
                print(f"  [SKIP] {out_dir}: missing gnn_node_index_master.csv")
...
def main():
    print("=" * 70)
    print(f"BOOST EXPLORATION: {int(DATA_FRAC*100)}% data, 10 configs × 3 datasets")
    print("Target: beat MAE 0.006161 | Depth | Norm | Phase onehot | Huber | LR scheduler")
    ...
    for cfg in CANDIDATES:
        cfg_name, n_emb, e_emb, h_dim, n_layers, use_norm, use_phase_onehot, use_huber = cfg
        for out_dir, feature_cols, ds_label in DATASETS:
            print(f"\n>>> {cfg_name} + {ds_label} (n={n_layers} h={h_dim} norm={use_norm} phase_oh={use_phase_onehot} huber={use_huber})")
            try:
                mae, rmse, best_ep = train_one(out_dir, feature_cols, ds_label, cfg_name, n_emb, e_emb, h_dim, n_layers,
                                                use_norm, use_phase_onehot, use_huber, data_frac=DATA_FRAC)
                if mae is not None:
