...
Train two models on the same 10k samples from gnn_samples_deltav_full:
  (A) 14 features: loadtype (13) + vmag_zero_pv_pu, target vmag_pu, full graph
  (B) Phase A only: 13 features (no vmag_zero), target vmag_pu, phase A subgraph
Same data. Comparison and worst-node selection only on phase A nodes.
...
def train_one(X_all, Y_all, edge_index, edge_attr, edge_id, N, E, node_in_dim, ckpt_path, name):
    ...
    ckpt = {
        ...
        "config": {"N": N, "E": E, "node_in_dim": node_in_dim, "edge_in_dim": 2, "out_dim": 1,
                   ...
                   "target_col": "vmag_pu", "dataset": DIR_DELTAV, "phase_a_only": False},
...
def train_phase_a_only(X_all, Y_all, phase_map_np, phase_edges, N, ckpt_path):
    """Train single GNN on phase A nodes only."""
    ...
    X_ph = X_all[:, phase_a_indices, :]
    Y_ph = Y_all[:, phase_a_indices, :]
...
def run_24h_and_plot():
    ...
    model_14 = PFIdentityGNN(num_nodes=N, num_edges=int(cfg_14["E"]), node_in_dim=14, edge_in_dim=2, out_dim=1,
                             node_emb_dim=8, edge_emb_dim=4, h_dim=32, num_layers=4, use_norm=False).to(DEVICE)
    ...
    for t in range(NPTS):
        ...
        x_14 = torch.tensor(X_14, dtype=torch.float32, device=DEVICE)
        x_ph = torch.tensor(X[phase_a_indices, :], dtype=torch.float32, device=DEVICE)

        g_14 = Data(x=x_14, edge_index=ei, edge_attr=ea, edge_id=eid, num_nodes=N)
        g_ph = Data(x=x_ph, edge_index=ei_ph, edge_attr=ea_ph, edge_id=eid_ph, num_nodes=N_phase_a)

        with torch.no_grad():
            V_14[t, :] = model_14(g_14)[:, 0].cpu().numpy()
    ...
    for i in range(N):
        ...
        if np.sum(ok) > 0:
            ...
            if i in phase_a_set and np.any(np.isfinite(V_ph[ok, i])):
    ...
    for k, idx in enumerate(worst_indices):
        print(f"  {k+1}. {node_names_master[idx]}: 14feat MAE={mae_14[idx]:.4f} | phase-A MAE={mae_ph[idx]:.4f}")
    ...
    for k, idx in enumerate(worst_indices):
        ...
        ax.plot(t_hours, V_14[:, idx], color="orange", linestyle="--",
                label=f"14 feat (MAE={mae_14[idx]:.4f})", linewidth=1.5)
        ax.plot(t_hours, V_ph[:, idx], "g:", label=f"Phase A only (MAE={mae_ph[idx]:.4f})", linewidth=1.5)
...
def main():
    print("=" * 70)
    print("14 FEAT (loadtype+vmag_zero) vs PHASE A ONLY: same 10k samples from deltav")
    print("=" * 70)
    ...
    train_one(X_14, Y_all, edge_index, edge_attr, edge_id, N, E, 14, CKPT_14FEAT,
              "14 feat (loadtype+vmag_zero)")
    train_phase_a_only(X_13, Y_all, phase_map_np, phase_edges, N, CKPT_PHASE_A)
