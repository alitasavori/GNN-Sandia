...
class case(object):
    def __init__(self, file,sourcebus,refvm,refva):
        ...
        # Prefer parser-resolved casing (e.g. SourceBus vs sourcebus).
        case.sourcebus = getattr(self, "refbus", sourcebus)
    ...
    def get_transformer_current(self, name, pu):
        """Restored from upstream Log-v-3LPF (needed by Controllers.apply_regcontrols)."""
        hit = self.transformers.index[
            self.transformers.name.astype(str).str.lower() == str(name).lower()
        ]
        if len(hit) == 0:
            return None
        idx = int(hit[0])
        if self.transformers.windings[idx] == 3:
            print("Calculation not implemented")
            return None
        fbus, tbus = self.transformers.buses[idx][0], self.transformers.buses[idx][1]
        fphase, tphase = self.transformers.phases[idx][0], self.transformers.phases[idx][1]
        fidx = [self.bus_phases[fbus].index(i) for i in fphase]
        ...
        Vn = np.array(
            [
                polar_to_cartesian(
                    self.results["logv3lpf"]["vm"][fbus][i],
                    self.results["logv3lpf"]["va"][fbus][i],
                )
                for i in fidx
            ]
        )
        Vm = np.array(
            [
                polar_to_cartesian(
                    self.results["logv3lpf"]["vm"][tbus][i],
                    self.results["logv3lpf"]["va"][tbus][i],
                )
                for i in tidx
            ]
        )
        ...
        if pu:
            return I
        scaling = np.diag(
            [self.base[fbus]["IBase"]] * len(fidx)
            + [self.base[tbus]["IBase"]] * len(tidx)
        )
        return scaling @ I
