"""
Unidirectional dataset + train + evaluate: edges only upstream→downstream (power-flow direction).
Subsets loadtype data to ~20k samples, creates unidirectional edges via electrical distance,
trains a model, saves it, and evaluates 24h profile vs OpenDSS at a specific node.
...
def evaluate_profile(obs_node=OBSERVED_NODE):
    """Run 24h profile: OpenDSS vs GNN at obs_node, plot overlay, report MAE/RMSE."""
    ...
    for t in range(NPTS):
        ...
        g = Data(x=torch.tensor(X, dtype=torch.float32, device=DEVICE), edge_index=edge_index, edge_attr=edge_attr, edge_id=edge_id, num_nodes=N)
        with torch.no_grad():
            yhat = model(g)
    ...
    ax.plot(t_hours, vmag_gnn, label="GNN unidirectional |V| (pu)")
    ax.set_xlabel("Hour of day")
    ax.set_ylabel("Voltage magnitude (pu)")
    ax.set_title(f"Unidirectional GNN vs OpenDSS @ {obs_node} (24h) | MAE={mae:.6f} RMSE={rmse:.6f}")
    ax.grid(True)
    ax.legend()
    plt.tight_layout()
    plot_path = os.path.join(OUTPUT_DIR, "overlay_24h_unidir.png")
    fig.savefig(plot_path, dpi=150, bbox_inches="tight")
    plt.show()
    plt.close()
    print(f"  [saved] {plot_path}")
    print(f"  @ {obs_node}: MAE={mae:.6f} pu | RMSE={rmse:.6f} pu")
    return mae, rmse


def main():
    print("=" * 70)
    print("UNIDIRECTIONAL: Dataset + Train + Evaluate")
    print("=" * 70)
