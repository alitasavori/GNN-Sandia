...
def _prepare_chunk_streaming(
    *,
    ...
    print(f"[chunk_parent] {len(chunk_dirs)} chunks under {chunk_parent} (streaming, O(1 chunk) RAM)", flush=True)
    for ch in chunk_dirs:
        print(f"  - {ch.name}", flush=True)
    ...
    for ci, ch in enumerate(chunk_dirs):
        ...
        full_sids = sorted(int(x) for x in pd.read_csv(nodes_path, usecols=["sample_id"])["sample_id"].unique().tolist())
        if float(sample_frac) >= 1.0:
            selected_sids: list[int] | None = None
        else:
            selected_sids = _select_chunk_sample_ids(full_sids, float(sample_frac), int(seed), int(ci))
        cache_pt = _chunk_cache_path(cache_dir, ch.name, float(sample_frac), int(seed), int(ci))
        cache_pts.append(cache_pt)
        selected_ids_list.append(selected_sids)

        x, y_ri, _sids, ntl = _ensure_chunk_tensor_cache_gnn(
            ch,
            nodes_name=nodes_csv_name,
            node_feature_cols=node_feature_cols,
            selected_sample_ids=selected_sids,
        ...
        if ref_ntl is None:
            ...
            if edge_shared_csv is not None:
                print(f"Using shared edges from {edge_path}", flush=True)
            edge_index, edge_attr = _load_compacted_edges(edge_path, ref_ntl)
            ...
            sum_y = torch.zeros(int(x.shape[1]) * 2, dtype=torch.float64)
            sum_y2 = torch.zeros(int(x.shape[1]) * 2, dtype=torch.float64)
        else:
            if ntl != ref_ntl:
                raise RuntimeError(f"{ch.name}: node_to_local mismatch vs first chunk")

        n = int(x.shape[0])
        n_samples += n
        rng = np.random.default_rng(int(seed) + ci * 100_003)
        ...
        idx_train_list.append(perm[:n_tr])
        idx_val_list.append(perm[n_tr : n_tr + n_va])
        idx_test_list.append(perm[n_tr + n_va :])

        itr = idx_train_list[-1]
        xt = x[itr].reshape(-1, n_node_features).to(dtype=torch.float64)
        assert sum_x is not None and sum_x2 is not None and sum_y is not None and sum_y2 is not None
        sum_x += xt.sum(dim=0)
        ...
        del x, y_ri
        gc.collect()
        print(f"[chunk {ci+1}/{len(chunk_dirs)}] samples={n} (norm stats)", flush=True)

    assert ref_ntl is not None and edge_index is not None and edge_attr is not None
    assert sum_x is not None and sum_x2 is not None and sum_y is not None and sum_y2 is not None and cnt_x > 0
    ...
    n_train = sum(len(a) for a in idx_train_list)
    n_val = sum(len(a) for a in idx_val_list)
    n_test = sum(len(a) for a in idx_test_list)

    return ChunkStreamingContext(
...
def _train_one_chunk_streaming(
    *,
    model_type: str,
    args: argparse.Namespace,
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for j in train_order:
            ...
            if node_pe_path is not None:
                x = _append_shared_pe_features(
                    x, node_to_local=ctx.ref_ntl, node_pe_csv=node_pe_path, node_pe_cols=node_pe_cols, verbose=False
                )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="If set, trains on all run_* chunks. Uses O(1-chunk) RAM (streamed) unless --cache_tensor points to a single packed .pt.",
    )
...
def main() -> None:
    ...
    if tf <= 0 or vf <= 0 or tf + vf >= 1.0:
        raise ValueError(
            ...
            "so some samples remain for the test set. "
            f"Got train_frac={tf}, val_frac={vf} (sum={tf + vf}). "
            "Example: --train_frac 0.9 --val_frac 0.09 (~10% test)."
        )
    ...
    if chunk_parent is not None and not use_monolithic_chunk:
        eff_cache_dir = cache_dir if cache_dir is not None else (out_dir / "chunk_tensor_cache_gnn_only")
        ...
        for m in models:
            ...
            res = _train_one_chunk_streaming(
                model_type=m,
                args=args,
                ctx=ctx,
                node_pe_path=node_pe_path,
